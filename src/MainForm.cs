...
namespace ExcelTrainingMonitor
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            dgvAlerts.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(45, 45, 48);
            dgvAlerts.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            dgvAlerts.BackgroundColor = Color.FromArgb(30, 30, 30);
            dgvAlerts.DefaultCellStyle.BackColor = Color.FromArgb(37, 37, 38);
            dgvAlerts.DefaultCellStyle.ForeColor = Color.White;
            dgvAlerts.GridColor = Color.Black;
            ...
            dgvAlerts.DefaultCellStyle.SelectionBackColor = Color.FromArgb(70, 70, 70);
        ...
        private void RunScan()
        {
            ...
            if (newAlerts.Count > 0)
            {
                foreach (var alert in newAlerts)
                {
                    ...
                    NotificationManager.ShowNotification("NEW TRAINING ALERT", message);
                }

                ForceForeground();
            }

            int reminderHours = (int)numReminderHours.Value;

            if (MinderScheduler.FileNeedsReminder(excelPath, reminderHours))
            {
                NotificationManager.ShowNotification("Reminder", "Excel file has not been updated.");
            }
        ...
        FileSystemEventArgs e)
        {
            try
            {
                ...
                {
                    RunScan();
                    NotificationManager.ShowNotification("Excel Updated", "Training matrix changed.");
                }));
            }
            catch
            {
